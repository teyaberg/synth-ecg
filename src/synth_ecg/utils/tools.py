...
DowerMatrix = np.array(
    [
        [0.632, 0.235, -0.397, -0.434, 0.515, -0.081, -0.515, 0.044, 0.882, 1.213, 1.125, 0.831],
...
def Rx(x):
    ...
    return np.array([[1, 0, 0], [0, np.cos(th_x), -np.sin(th_x)], [0, np.sin(th_x), np.cos(th_x)]])


def Ry(y):
    th_y = y * np.pi / 180.0

    return np.array([[np.cos(th_y), 0, np.sin(th_y)], [0, 1, 0], [-np.sin(th_y), 0, np.cos(th_y)]])


def Rz(z):
    th_z = z * np.pi / 180.0

    return np.array([[np.cos(th_z), -np.sin(th_z), 0], [np.sin(th_z), np.cos(th_z), 0], [0, 0, 1]])


# TODO don't take first 10 seconds, or at least exclude the first cardiac cycle
# solve input ode object
def solve_vcg_object(vcg_ode, fs=512, duration=10, v0=np.array([0, 0.3, 0.3, 0.3])):
    duration += 10
...
def plot12(ecg, fs=512, n_squares=8):
    ...
    majorgrid_x = np.linspace(0, 10, int(10 / 0.2))
    minorgrid_x = np.linspace(0, 10, int(10 / 0.04))

    i = 0
    for a in axs.flatten():
        majorgrid_x = np.linspace(
            0.1 * ((i % 4) == 3) + 0.1 * ((i % 4) == 1),
            10 + 0.1 * ((i % 4) == 3) + 0.1 * ((i % 4) == 1),
            int(10 / 0.2) + 1,
        )
        minorgrid_x = np.linspace(
            0.0 + 0.02 * ((i % 4) == 3) + 0.02 * ((i % 4) == 1),
            10.0 + 0.02 * ((i % 4) == 3) + 0.02 * ((i % 4) == 1),
            int(10 / 0.04) + 1,
        )
    ...
    majorgrid_x = np.linspace(0, 10, int(10 / 0.2) + 1)
    minorgrid_x = np.linspace(0, 10, int(10 / 0.04) + 1)
